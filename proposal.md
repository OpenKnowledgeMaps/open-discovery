...
###A roadmap for developing a visual interface to the world's scientific knowledge.
...
In the first phase, we will extend both the backend and the frontend of the existing knowledge mapping software. 

####Expand Content Sources
One of the primary objectives is to extend the existing software to incoporate further open content sources, including non-publication content. We use many software packages produced by [rOpenSci](http://ropensci.org), including the ability to search for scholarly content against the following engines:
...
##Licensing
The code will be made available on Github under the license of the existing knowledge mapping software (LGPL v3). The visualizations will be released under CC-BY 4.0 - with the exception of the contained content, which of course retains its original license.
